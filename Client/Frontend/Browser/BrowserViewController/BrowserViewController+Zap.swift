...
class ZapAnimationController: UIViewController {
    ...
    private lazy var topZap: UIImageView = .build { imageView in
        imageView.image = UIImage(named: "qwant_zap_top_2")
        imageView.alpha = 0.0
    }

    private lazy var bottomZap: UIImageView = .build { imageView in
        imageView.image = UIImage(named: "qwant_zap_bottom_2")
        imageView.alpha = 0.0
    }
    ...
    // Internal
    private var zapIsFinished: Bool = false
    ...
    private func in1_fadeInZoomInYellowView(completion: @escaping () -> Void) {
        ...
        UIView.animate(withDuration: UX.duration, delay: 0.0, options: .curveEaseIn, animations: {
