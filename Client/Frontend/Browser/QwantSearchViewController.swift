...
protocol QwantSearchViewControllerDelegate: AnyObject {
    func qwantSearchViewController(_ searchViewController: QwantSearchViewController, didSelectURL url: URL, searchTerm: String?)
    ...
    func qwantSearchViewController(_ searchViewController: QwantSearchViewController, didHighlightText text: String, search: Bool)
...
class QwantSearchViewController: UIViewController,
    ...
    lazy var tableView: UITableView = {
        ...
        table.register(QwantSearchTableViewHeader.self, forHeaderFooterViewReuseIdentifier: QwantSearchTableViewHeader.cellIdentifier)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = getCellForSection(twoLineImageOverlayCell,
                                     ...
                                     indexPath)
        cell.backgroundColor = themeManager.currentTheme.colors.omnibar_tableViewCellBackground(viewModel.isPrivate)
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        guard section == SearchListSection.suggest.rawValue,
        ...
        headerView.applyUIMode(isPrivate: self.viewModel.isPrivate, theme: themeManager.currentTheme)
        return headerView
    }
    ...
    private func getCellForSection(_ twoLineCell: TwoLineImageOverlayCell,
        ...
        switch section {
            case .suggest:
                ...
                oneLineCell.leftImageView.image = UIImage(named: "qwant_search")?.createScaled(CGSize(width: 20, height: 20)).withRenderingMode(.alwaysTemplate)
                oneLineCell.leftImageView.backgroundColor = themeManager.currentTheme.colors.omnibar_tintColor(viewModel.isPrivate).withAlphaComponent(0.2)
                oneLineCell.leftImageView.tintColor = themeManager.currentTheme.colors.omnibar_tintColor(viewModel.isPrivate)
            ...
            case .openedTabsAndBookmarks:
                ...
                if let openedTab = tabsAndBookmarks[indexPath.row] as? Tab {
                    ...
                    twoLineCell.descriptionLabel.text = [openedTab.url?.normalizedHost, String.SearchSuggestionCellSwitchToTabLabel].compactMap({ $0 }).joined(separator: " • ")
                    twoLineCell.leftOverlayImageView.image = UIImage(named: "qwant_tabs")?.withRenderingMode(.alwaysTemplate)
                    twoLineCell.leftOverlayImageView.tintColor = themeManager.currentTheme.colors.omnibar_tintColor(viewModel.isPrivate)
                    twoLineCell.leftOverlayImageView.backgroundColor = themeManager.currentTheme.colors.omnibar_tableViewCellBackground(viewModel.isPrivate)
                    ...
                    twoLineCell.leftOverlayImageView.image = UIImage(named: "qwant_bookmarks")?.withRenderingMode(.alwaysTemplate)
                    twoLineCell.leftOverlayImageView.tintColor = themeManager.currentTheme.colors.omnibar_tintColor(viewModel.isPrivate)
                    twoLineCell.leftOverlayImageView.backgroundColor = themeManager.currentTheme.colors.omnibar_tableViewCellBackground(viewModel.isPrivate)
        ...
        oneLineCell.selectedBackgroundView?.backgroundColor = theme.colors.omnibar_tableViewSelectedCellBackground(viewModel.isPrivate)
        ...
        twoLineCell.selectedBackgroundView?.backgroundColor = theme.colors.omnibar_tableViewSelectedCellBackground(viewModel.isPrivate)
        return cell
    }
...
extension QwantSearchViewController {
    func handleKeyCommands(sender: UIKeyCommand) {
        ...
        guard let firstSection = SearchListSection.allCases.first(where: { tableView(tableView, numberOfRowsInSection: $0.rawValue) > 0 })?.rawValue else {
            return
        }

        guard let lastSection = SearchListSection.allCases.last(where: { tableView(tableView, numberOfRowsInSection: $0.rawValue) > 0 })?.rawValue else {
            return
        }
