        ...
        { result in
            ...
            let sites = filterHistory.compactMap {
                ...
                if $0.siteUrl?.isQwantUrl == true, let term = $0.siteUrl?.qwantSearchTerm, !term.isEmptyOrWhitespace() {
                    if qwantSearches.contains(term) { skip = true }
                    else { qwantSearches.append(term) }
                }
                ...
                return !skip && filters.contains { $0.lowercased().contains(query) } ? Site(url: $0.url, title: $0.displayTitle) : nil
            }
            
            completion(Array(sites.prefix(maxCount)))
        }
    }
}
