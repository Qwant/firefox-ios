...
class QwantZapSettingsTableViewController: ThemedTableViewController {
    ...
    fileprivate var zapButtonEnabled = true {
        didSet {
            let warningColor = themeManager.getCurrentTheme(for: windowUUID).colors.textCritical
            let disabledColor = themeManager.getCurrentTheme(for: windowUUID).colors.textDisabled
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == SectionToggles {
            ...
            control.onTintColor = themeManager.getCurrentTheme(for: windowUUID).colors.actionPrimary
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == SectionButton {
            ...
            alert.addAction(UIAlertAction(title: .QwantZap.ZapAlertOK, style: .destructive, handler: { [weak self] _ in
                ...
                let theme = self.themeManager.getCurrentTheme(for: windowUUID)
